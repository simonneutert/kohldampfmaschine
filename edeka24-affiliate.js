  ...
  function runComparisonScore(data, word) {
    for (let element in data) {
      for (let productNameWord of data[element].name.split(' ')) {
        ...
        let shorterWord = word.toLowerCase();
        let longerWord = productNameWord.toLowerCase();
        if (shorterWord.length > longerWord.length) {
          shorterWord = productNameWord.toLowerCase();
          longerWord = word.toLowerCase();
        }

        let match = longerWord.match(shorterWord);
        if (!match) {
          continue; // guard clause
        }
